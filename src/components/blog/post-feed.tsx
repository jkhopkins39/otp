...
import { deletePostAction, setFeaturedPostAction } from "@/app/admin/actions";
...
export function PostFeed({
  ...
  function handleToggleFeatured(id: string) {
    startTransition(async () => {
      await setFeaturedPostAction(id);
      router.refresh();
    });
  }
    ...
    <div>
      ...
      {admin && (
        <p className="mb-4 text-xs text-muted-foreground">
          Click the <span className="font-medium text-gold-strong">★</span> star to pin a post as the featured one on the homepage.
        </p>
      )}
        ...
        <div className="flex flex-col gap-5">
          {posts.map((post) => (
            <PostCard
              ...
              onToggleFeatured={admin ? handleToggleFeatured : undefined}
            />
          ))}
        </div>
      )}
